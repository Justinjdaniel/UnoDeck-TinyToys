...
export const Board: React.FC<BoardProps> = ({
  ...
  return (
    <div className="mobile-viewport select-none flex flex-col justify-between bg-slate-950 text-white overflow-hidden relative">
      {/* Outer game board content wrapper, hidden and inert when Pass & Play modal is open */}
      <div
            ...
            {isMyTurn ? (
              gameState.selectedWildCard ? (
                ...
                <span className="text-xs font-bold text-emerald-400">
                  {isPassAndPlay ? `${activePlayer.name}'s turn!` : 'Your turn!'} Play a matching
                  card or draw.
                </span>
              )
            ) : (
      ...
      {/* 5. Pass & Play Interstitial Hand Overlay (Accessible Dialog) */}
      {showPassOverlay && (
        <div
